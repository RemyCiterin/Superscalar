...
module mkDCache#(Bit#(sourceW) source) (DCache#(sizeW, sourceW, sinkW));
  ...
  if (useCoherency) rule probe_match if (state[0] == MatchingProbe);
  ...
  if (useCoherency) rule probe_evict if (state[0] == EvictProbe);
  ...
  rule mis if (state[0] == Lookup && !cpu_hit && queueA.canEnq);
    ...
    if (needEvict) begin
      ...
      state[0] <= Refill;
      queueA.enq(ChannelA{
        opcode: useCoherency ? AcquireBlock(grow) : GetFull,
  ...
  rule receive_evict_resp if (state[0] == EvictWait);
    ...
    state[0] <= Refill;
    queueA.enq(ChannelA{
      opcode: useCoherency ? refill_opcode : GetFull,
      address: pack(Physical{ tag: cpu_phys.tag, index: cpu_phys.index, offset: 0, lsb: 0 }),
      size: fromInteger(lineSize),
      source: source,
      mask: -1,
  ...
  rule receive_beat if (state[0] == Refill);
    ...
    if (refill_offset + 1 == 0 || !hasDataD(queueD.first.opcode)) begin
      ...
      TLPerm perm = useCoherency ? (exclusive ? D : B) : T;
      permRams[cpu_way].writePorts[0].request(cpu_phys.index, perm);
